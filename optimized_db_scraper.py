...
def create_database_table():
    """Create the Form_F2_Data table in the database if it doesn't exist"""
    try:
        ...
        cursor.execute("""
            IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Form_F2_Data]') AND type in (N'U'))
            BEGIN
                ...
                PRINT 'Form_F2_Data table created successfully'
            ...
            ELSE
            BEGIN
                PRINT 'Form_F2_Data table already exists'
            END
        """)
        ...
        print("Database table setup successful")
        return True
    except Exception as e:
        print(f"Error creating database table: {str(e)}")
        return False

class OptimizedDatabaseScraper(FamilyPlanningDataFetcher):
    """FamilyPlanningDataFetcher with optimized database operations and local progress tracking"""
    ...
    def __init__(self, start_date="2016-12", end_date="2025-01", max_workers=4, max_retries=5):
        ...
        self.db_logger = setup_logging("OptimizedDBScraper")
...
def main():
    parser = argparse.ArgumentParser(description="Optimized Family Planning Database Scraper")
    ...
    print(f"Optimized Family Planning Database Scraper")
    print(f"==============================================")
    ...
    try:
        fetcher = OptimizedDatabaseScraper(
        ...
        print(f"python optimized_db_scraper.py")
