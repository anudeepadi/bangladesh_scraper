# PowerShell script to run the Optimized Family Planning Database Scraper
...
if (-not $skipTableCreation) {
    ...
    if ($sqlcmdExists) {
        ...
        sqlcmd -S $env:SERVER -d $env:BANGLADESH_DATABASE -U $env:UID -P $env:PID -i create_form_f2_table.sql
    } else {
...
# Start the scraper
Write-Host "Starting optimized database scraper..." -ForegroundColor Green
...
Write-Host "Running: python optimized_db_scraper.py $arguments" -ForegroundColor Cyan
Invoke-Expression "python optimized_db_scraper.py $arguments"
